# https://github.com/aitechlabcn/neUNet/tree/main
...
from utils.losses import MemoryEfficientSoftDiceLoss
...
class neUnetDecoder(nn.Module):
    ...
    def forward(self, skips):
        ...
        for s in range(len(self.stages)):
            ...
            x = torch.cat((pad2same(x, skips[-(s + 2)]), skips[-(s + 2)]), 1)
            x = self.stages[s](x)
            if self.deep_supervision:
                seg_outputs.append(self.seg_layers[s](x))
            elif s == (len(self.stages) - 1):
...
class NeUNet(PlainConvUNet, BaseModel):
    def __init__(self,
                 input_channels: int,
                 n_stages: int,
                 features_per_stage: Union[int, List[int], Tuple[int, ...]],
    ...
    def backward(self, x, optimer, closure=None, clear_stored=True):
        default = LossWrap({
            ...
            'dice': {
                'loss': MemoryEfficientSoftDiceLoss(ddp=False, with_logits=True),
                'args': {},
                'weight': 1.0
            }
        })
