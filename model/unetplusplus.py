...
class UNetPlusPlus(BaseModel):
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        self.td = nn.ModuleList(self.td) # ??? where
    ...
    def forward(self, x):
        ...
        if self._deep_supervision and self.do_ds:
            hidden = [i(j) for i, j in
                                              zip(list(self.upscale_logits_ops)[::-1], seg_outputs[:-1][::-1])]
            self.pre['hidden'] = hidden
        ...
        def closure_(pre):
            with torch.enable_grad():
                if closure is not None:
                    ...
                    loss_total.backward()
                    nn.utils.clip_grad.clip_grad_norm_(self.parameters(), 12)
                    optimer.step()
                    return loss
...
# UNetPlusPlus(3, 64, 1, 4, norm_op_kwargs={'eps': 1e-5, 'affine': True}, dropout_op_kwargs={'p': 0., 'inplace': False}, 
#              pool_op_kernel_sizes=4, conv_kernel_sizes=3, convolutional_upsampling=True)