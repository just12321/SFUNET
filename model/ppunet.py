# https://github.com/tljxyys/Perspective-Unet/tree/main
...
from model.modules import DropPath
from einops import rearrange, repeat
from functools import partial
from model.modules import _pair
...
class CNNEncoder(nn.Module):
    def __init__(self, in_channels, use_dilate_conv=True):
...
class TransformerEncoder(nn.Module):
    def __init__(self, size=[128, 64, 32]):
...
class SpatialTransformerEncoder(nn.Module):
    def __init__(self, d_model=128, nhead=8, num_layers=6, dim_feedforward=1024, dropout=0.1):
        super().__init__()
...
class SpatialCrossScaleIntegrator(nn.Module):
    def __init__(self, channel_sizes, size, target_dim=128, nhead=8, depth=6):
        super().__init__()
        ...
        self.h_w = [_pair(size >> 2), _pair(size >> 3), _pair(size >> 4)]  # Hardcoded sizes for outputs
...
class Encoder(nn.Module):
    def __init__(self, in_channels, size, use_enltb=True, use_scale_integrator=True, use_dilate_conv=True):
        super(Encoder, self).__init__()
        self.Encoder1 = CNNEncoder(in_channels, use_dilate_conv=use_dilate_conv)
        self.Encoder2 = TransformerEncoder([size >> 2, size >> 3, size >> 4]) if use_enltb else None
        self.num_module = 3
        self.fusion_list = [128, 256, 512]
        self.use_scale_integrator = use_scale_integrator
        self.scale_integrator = SpatialCrossScaleIntegrator(self.fusion_list, size) if use_scale_integrator else None
...
class Decoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.first_block = self.LastDecoderBlock(512, self.encoder_channels[0])
...
class PPUnet(BaseModel):
    def __init__(self, in_channels, num_classes, size, use_enltb=True, use_scale_integrator=True, use_dilate_conv=True):
        super().__init__()
        self.encoder = Encoder(in_channels, size, use_enltb=use_enltb, use_scale_integrator=use_scale_integrator, use_dilate_conv=use_dilate_conv)
        self.decoder = Decoder()
        self.segmentation_head = SegmentationHead(in_channels=32, out_channels=num_classes, kernel_size=3,)
